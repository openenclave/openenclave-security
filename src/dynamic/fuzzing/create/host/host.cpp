...
char kEnclavePath[] = "/tmp/2de32e3b-9225-4be9-a7b7-56b2e73c4448";
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    if (size > 64)
    {
        ...
        {
            std::ofstream enc;
            enc.open(kEnclavePath, std::ios::app);
            handle = dlopen("libcreate_fileapihook.so", RTLD_LOCAL | RTLD_LAZY);
            if (handle)
            {
                init_file_hook = (fn_init_file_hook)dlsym(handle, "init_file_hook");
            }
        }
        ...
        if (init_file_hook)
        {
            ...
            char *enc_path = kEnclavePath;
            (*init_file_hook)(data, size);
            (void)CreateEnclave(enc_path);
        }
    }
